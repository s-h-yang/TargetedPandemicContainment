...
function get_list_of_weights_sorted_nodes(ll::Vector{Vector{Int}},
            sorted_nodes::Vector{Int}, edge_to_loc::Dict{Tuple{Int,Int},Int},
            target_perc::Float64; weight::Float64=0.1)
...
function get_list_of_weights_random_nodes(ll::Vector{Vector{Int}},
            edge_to_loc::Dict{Tuple{Int,Int},Int}, target_perc::Float64;
            weight::Float64=0.1)

    lw::Vector{Vector{Float64}} = [ones(Float64,length(l)) for l in ll]
    n = length(ll)
    selected_nodes = rand(1:n, Int(ceil(n*target_perc)))
...
function get_list_of_weights_node_centrality_target_nodes(ll::Vector{Vector{Int}},
            node_centrality::Vector{Float64}, edge_to_loc::Dict{Tuple{Int,Int},Int},
            target_perc::Float64; weight::Float64=0.1)

    lw::Vector{Vector{Float64}} = [ones(Float64,length(l)) for l in ll]
    n = length(ll)
    sorted_node_indices = sortperm(node_centrality, rev=true)
    ct = 0
...
function edge_to_node_betweenness(edges, edge_betweenness, n)
    node_betweenness = zeros(Float64, n)
    for (idx,val) in enumerate(edge_betweenness)
        u,v = edges[idx]
        node_betweenness[u] += val
        node_betweenness[v] += val
    end
